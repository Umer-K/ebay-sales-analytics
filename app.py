...
def parse_sales_data(file_content):
    ...
    try:
        ...
        if num_cols >= 7:
            # Check multiple rows (first 10) to detect format more reliably
            price_indicators = 0
            rows_to_check = min(10, len(df))
            
            for i in range(rows_to_check):
                sample_val = str(df.iloc[i, 2]).strip()
                # Check if it looks like a price (has $ or is a decimal number)
                if '
        ...
        if format_type == 'new' and num_cols >= 7:
            ...
            st.sidebar.success(f"✅ Detected NEW format (with Price) - {len(df)} rows")
        elif num_cols >= 6:
            ...
            df['Price'] = 0.0
            st.sidebar.warning(f"⚠️ Detected OLD format (no Price) - {len(df)} rows")
        elif num_cols >= 5:
            df.columns = ['Product', 'URL', 'Dec 2025 Sales', 'Jan 2026 Sales', 'Date Checked'] + [f'Extra_{i}' for i in range(num_cols - 5)]
            df['Price'] = 0.0
            df['Status'] = 'N/A'
            st.sidebar.warning(f"⚠️ Detected minimal format - {len(df)} rows")
        else:
        ...
        # Convert price to numeric, handling currency symbols
...
if uploaded_file is not None:
    ...
    **Note:** Both formats can be mixed in the same file! The dashboard auto-detects the format.
    """) in sample_val:
                    price_indicators += 1
                elif sample_val.replace('.', '').replace(',', '').isdigit() and '.' in sample_val:
                    # Check if it's a price-like number (not 0 or 1 which are more likely sales)
                    try:
                        val = float(sample_val.replace(',', ''))
                        if val > 2:  # Prices are usually > $2
                            price_indicators += 1
                    except:
                        pass
            
            # If more than 30% of checked rows look like prices, use new format
            if price_indicators > rows_to_check * 0.3:
